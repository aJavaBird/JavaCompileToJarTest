...
import com.haizhi.huaxing.http.xml.vo.SvcBodyUUS1018T;

public class HttpRequestUUS1018TTest {
    public static void main(String args[]) {

        EsbMessage esb = getTestEsbMessage();

        System.out.println("--------------------------------------");
        System.out.println("域账户秘密验证接口:UUS1018T 测试开始~");

//        String url = "http://10.1.89.195:9807";
        String ip = "10.1.89.195";
//        String url = "http://10.1.95.25:9807";
//        String ip = "10.1.95.25";
        Integer port = 9807;
...
//        EsbRequestUtil.sendXmlTcp(esb,ip,port);
        ...
        System.out.println("令牌验证接口:UUS1018T 测试结束~");
        System.out.println("--------------------------------------");
    ...
    private static EsbMessage getTestEsbMessage() {
        ...
        header.setTranCode("001018"); // 交易码，不同的接口交易码不同
        String seqNo = Uuid.get16UUID("HAIZHI");
        header.setTranSeqNo(seqNo); // 自己生成的id，只要不重复就行，长度不超过32位
        header.setGlobalSeqNo(seqNo); // 自己生成的id，只要不重复就行，长度不超过23位
        header.setChannel("1001"); // 项目的渠道号（四位数字），每个项目可能不一样

        SvcBodyUUS1018T svcBody = new SvcBodyUUS1018T();
        ...
        svcBody.setUserNo("yw.xie"); // 用户名
        svcBody.setPassWord("55"); // 用户密码

        EsbMessage esb = new EsbMessage();
        esb.setEsbServiceName("UUS1018T"); // Esb接口service名称
